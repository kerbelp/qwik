...
export const serverQrl = <T extends (...args: any[]) => any>(qrl: QRL<T>): QRL<T> => {
  if (isServer) {
    if (typeof (globalThis as any)._qwikFunctionsMap === 'undefined') {
      (globalThis as any)._qwikFunctionsMap = new Map();
    }
    (globalThis as any)._qwikFunctionsMap.set(qrl.getHash(), qrl);
  }
  function client() {
    return $(async (...args: any[]) => {
      if (isServer) {
        ...
        const res = await fetch(path, {
          ...
          body: await _serializeData(filtered),
        });
        if (!res.ok) {
